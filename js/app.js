...
function roomCard(project, room) {
  ...
  return `<article class="compact-card clickable" data-go="project/${project.id}/${room.id}"><div class="card-top"><div><h2>${esc(room.name)}</h2><p>Количество стен: ${room.walls.length} · ${square(totals.area)}</p></div><b class="price">${money(totals.total)}</b></div><span class="edit-link">Открыть комнату <b>→</b></span></article>`;
}
...
function renderProject(project) {
  ...
  app.innerHTML = page('Объект', project.name, `<div class="client-line">${project.client ? `<span>Клиент: <b>${esc(project.client)}</b></span>` : ''}${project.phone ? `<span>${esc(project.phone)}</span>` : ''}<button class="text-button" data-action="edit-project">Изменить</button></div><section class="section-head"><div><h2>Комнаты</h2><p>Количество комнат: ${project.rooms.length}</p></div><button class="secondary" data-action="add-room">＋ Добавить</button></section><div class="card-list">${project.rooms.length ? project.rooms.map(room => roomCard(project, room)).join('') : empty('Здесь появятся комнаты', 'Добавьте первую комнату, затем стены и материалы.')}</div><details class="estimate"><summary><div><h2>Смета / Комплектация</h2><p>${estimate.groups.reduce((sum, group) => sum + group.rows.length, 0)} позиций</p></div><span>⌄</span></summary><div class="estimate-content">${estimateContent}<div class="estimate-total"><span>Итого по смете</span><strong>${money(estimate.total)}</strong></div></div></details><section class="total-panel"><p>Итог по объекту</p><div><span>Общая площадь</span><b>${square(totals.area)}</b></div><strong>${money(totals.total)}</strong></section>`, 'home');
}
